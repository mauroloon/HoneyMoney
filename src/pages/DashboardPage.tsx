  ...
  return (
    <>
      <div className="flex-1 overflow-y-auto no-scrollbar pb-24 md:pb-8">
        ...
        <div className="px-4 pt-4 space-y-4 md:px-8 md:pt-6 md:grid md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] md:gap-6 md:space-y-0 md:items-start">
          ...
          <div className="space-y-4">
            ...
            {/* Register button inline on desktop */}
            <button
              onClick={() => setShowAdd(true)}
              className="hidden md:flex w-full items-center justify-center gap-2 bg-primary text-white py-3.5 rounded-2xl shadow-fab font-bold text-sm active:scale-[0.98] transition-transform duration-100"
            >
          ...
          <div className="space-y-4">
            ...
            {expensesByCategory.length > 0 && (
              <section>
                ...
                <div className="flex gap-2.5 overflow-x-auto no-scrollbar pb-1 md:flex-wrap md:overflow-x-visible md:pb-0">
                  {expensesByCategory.slice(0, 6).map(({ category, total }) => (
                    <div key={category.id} className="flex-shrink-0 flex flex-col items-center gap-1.5 bg-white rounded-2xl px-3 py-3 shadow-card w-[78px] md:flex-shrink">
                      <div
      ...
      {/* FAB — mobile only (desktop has inline button) */}
      <button
        onClick={() => setShowAdd(true)}
        className="fixed bottom-24 right-5 flex items-center gap-2 bg-primary text-white px-5 py-3.5 rounded-full shadow-fab font-bold text-sm z-10 active:scale-95 transition-transform duration-100 md:hidden"
      >
        <span className="text-base font-black leading-none">+</span>
        Registrar
      </button>
